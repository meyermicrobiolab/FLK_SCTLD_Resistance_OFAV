...
antibac_rpkm <- read.delim("../data/FLK_prok_coassembly_BGC_antimicro_rpkm.tsv", sep = "\t", row.names = 1)

# Fix the column names to only extract the name of the sample
colnames(antibac_rpkm) <- str_split_i(str_split_i(colnames(antibac_rpkm), "[.]", 3), "_", 1)
```
...
First question: What are the types of BGC's in the coral holobiont?
- What are the antiSMASH "Types"?
- What are the closest hits in the MIBiG database? (chose the hit with the closest similarity score)
- What is the potential function of that closest hit based on the literature cited by MIBiG?

A potential useful graphic here could be a alluvial plot. This would show the annotations and how they split up into different functional groups. 
...
Are the compounds here more prevalent in the different fates compared to others?? Maybe just look for presence/absence rather than abundance since it is unclear what the actual activity would be. We are just looking for general abundance. 

I will use CoverM, which has a "contig" setting to investigate both the mean number of reads covering each position in the contig as well as the rpkm based abundances. From there, I will choose a threshold for presence/absence and calculate the prevalence of the BGC's in the different fates. I will look for which BGCs are more prevalent in anyt of the fates.  
I will use the mean coverage metric more than likely rather than the RPKM term. It seems a little more intuitive and I like that I could also set a threshold for what I will consider an acceptable mean coverage value. 

First - Need to manipulate the data. I need to put it in a presence/absence format. Maybe to start, I could just take a look at all the data. 
...
I want to see if any of the BGC's were significantly differentially prevalent between unaffected and diseased corals. 

I hypothesize that the healthy coral samples may have a greater prevalence of BGC's encoding potential antimicrobial or toxic compounds to help support the maintenance of the coral host and protect against infection. 
...
bgc_test <- bgc_all_data %>%
  ...
  group_by(Disease_on_colony_when_coring_binary, pres_abs_words) %>%
  summarize(count = length(pres_abs_words)) %>%
  tidyr::spread(Disease_on_colony_when_coring_binary, count) %>%
  dplyr::select(Unaffected, Diseased) %>%
  as.data.frame()
...
set.seed(100)
fisher_result <- fisher.test(bgc_test)
print(fisher_result)

bgc_all_data %>% filter(Contig == "k127_2826510")
```
...
perform_fisher_test <- function(gene_row) {
  ...
  contingency_table <- matrix(as.integer(gene_row[2:5]), nrow = 2) #make sure the table is an integer
...
BGC_fisher <- bgc_all_data %>%
  group_by(Contig, Disease_on_colony_when_coring_binary, pres_abs_words) %>%
  summarize(count = length(pres_abs_words)) %>%
  mutate(disease_presabs = paste0(Disease_on_colony_when_coring_binary,"_",pres_abs_words)) %>%
  ungroup() %>%
  dplyr::select(Contig, count, disease_presabs) %>%
  as.data.frame() 

bgcs_antimicrobials <- BGC_fisher$Contig

BGC_fisher_wide <- BGC_fisher %>%
  group_by(Contig) %>%
  dplyr::select(count, disease_presabs) %>%
  pivot_wider(names_from = disease_presabs, values_from = count, values_fill = 0)
  #mutate(keep = ifelse(High_Present + Low_Present == 0, FALSE, TRUE)) %>% # make a row ID'ing to keep or not some moduels
 # filter(keep == TRUE)

# Apply the function to each row (gene) in the table and perform a multiple test correction
p_values <- base::apply(BGC_fisher_wide, 1, perform_fisher_test)

p_adjusted_BH <- p.adjust(p_values, method = "BH")

# Combine gene names and p-values into a table
bgc_p_values <- data.frame(Contig = BGC_fisher_wide$Contig, Adj_P_Value = p_adjusted_BH) %>% 
...
antimicrobials_with_pval <- left_join(antimicrobials, bgc_p_values, by = "Contig")
...
```
Cool...looks like none of the biosynthetic gene clusters were significantly differentially prevalent between the diseased and healthy corals. The takeaway there is that both diseased and healthy corals harbor the BGCs that encode diverse antimicrobial functions. These clusters may help both maintain the coral holobiont through production of diverse antimicrobial compounds or even promote infection in diseased corals. 

## Fate - Fisher's Exact Test
...
Before the adjusted p value, there were 2 that were significantly differentially abundant. 

# Strict coverage cutoff

## Mean exploration stricter cutoff

Theres a lot of very low values. Try making presence/absence with anything above 0 being present.

```{r}
antibac_presabs_1 <- ifelse(antibac > 1, 1, 0)
```
...
```{r}
metadata$CoralID
colnames(antibac_presabs_1)

# Subset metadata and get it in the same order as the heatmap data
idx <- match(colnames(antibac_presabs_1), metadata$CoralID)
metadata_bgc_1 <- metadata[idx, ]

match(metadata_bgc_1$CoralID, colnames(antibac_presabs_1))

# Categorical information (e.g Sample Disease Status)
annotation_col_1 <- data.frame(
  Colony_Fate = metadata_bgc_1$Fate_after_SP1)
rownames(annotation_col_1) <- colnames(antibac_presabs_1)

annotation_col_1$Colony_Fate <- factor(annotation_col_1$Colony_Fate, levels = c("Unaffected", "Recovered", "Will be diseased", "Diseased"))
...
# Categorical annotation for rows (e.g. BGC types)
antimicrobials$Contig
rownames(antibac_presabs_1)

#antimicrobials$Contig_Compound <- paste0(antimicrobials$Contig," (", antimicrobials$Compound, ")")

# Get the rows to match with the BGC metadata
idx <- match(rownames(antibac_presabs_1), antimicrobials$Contig)
bgc_antimicrobial_info_1 <- antimicrobials[idx, ]

# Check it worked
match(bgc_antimicrobial_info_1$Contig, rownames(antibac_presabs_1))

# Rename the rows of the matrix by the new contig and compound name
antibac_presabs_forheatmap_1 <- antibac_presabs_1
rownames(antibac_presabs_forheatmap_1) <- bgc_antimicrobial_info_1$Contig_Compound

# Set annotation levels for contigs (General Activity of BGC)
annotation_row_1 <- data.frame(
  Activity_General = bgc_antimicrobial_info_1$Activity_general)

rownames(annotation_row_1) <- rownames(antibac_presabs_forheatmap_1)

annotation_row_1$Activity_General <- factor(annotation_row_1$Activity_General, levels = c("antimicrobial", 
                                                                          "antimicrobial, cytotoxic", 
                                                                          "antimicrobial precursor",
                                                                          "antimicrobial defense", 
                                                                          "antibiotic resistance",
...
# Generate heatmap with colored gene names
pheatmap(antibac_presabs_forheatmap_1, 
...
# Save heatmap to PDF
pdf("../figures/BGC_antimicrobial_heatmap_06.27.26_StrictCutoff1.pdf", width = 13, height = 9)
pheatmap(antibac_presabs_forheatmap_1, 
        annotation_row = annotation_row_1, 
        annotation_colors = ann_colors,
        annotation_col = annotation_col_1, 
...
Use a fisher's exact test to test that one gene is significantly differentially prevalent between the four colony fates.

A fisher's exact test works with presence/absence data and it also works well with small sample sizes. Since the recovered group is only 4 smaples, this is a better target than the chi squared test. 
```{r}
#Combine all the info
bgc_all_data_1 <- antibac_presabs_1 %>% 
  as_tibble() %>%
  mutate(Contig = rownames(antibac_presabs_1)) %>%
  left_join(antimicrobials, by = "Contig") %>%
  gather(key = "sample", value = "pres_abs", `1220`:N73) %>%
  mutate(pres_abs_words = ifelse(pres_abs == 1, "Present", "Absent")) %>%
  left_join(metadata_bgc_1, by = c("sample" = "CoralID"))

# Try with just one BGC to test out the Fisher's Exact Test to see that it works
bgc_test_1 <- bgc_all_data_1 %>%
  filter(Contig == "k127_2342471") %>%
  group_by(Fate_after_SP1, pres_abs_words) %>%
  summarize(count = length(pres_abs_words)) %>%
  tidyr::spread(Fate_after_SP1, count, fill = 0) %>%
  dplyr::select(Unaffected, Recovered, `Will be diseased`, Diseased) %>%
  as.data.frame()

rownames(bgc_test_1) <- c("Absent", "Present")

# Run the Fisher's exact test - testing the null of independence of rows and columns in the contingency table - null hypothesis - no difference between presence and absence of genes 
set.seed(100)
fisher_result_1 <- fisher.test(bgc_test_1, simulate.p.value = TRUE)
print(fisher_result_1)

fisher_result_1$p.value
```
...
BGC_fisher_fate_1 <- bgc_all_data_1 %>%
  group_by(Contig, Fate_after_SP1, pres_abs_words) %>%
  summarize(count = length(pres_abs_words), .groups = "drop") %>%
  mutate(fate_presabs = paste0(Fate_after_SP1,"_",pres_abs_words)) %>%
  dplyr::select(Contig, count, fate_presabs) %>%
  as.data.frame() 

bgcs_antimicrobials_1 <- BGC_fisher_fate_1$Contig

BGC_fisher_fate_wide_1 <- BGC_fisher_fate_1 %>%
  group_by(Contig) %>%
  dplyr::select(count, fate_presabs) %>%
  pivot_wider(names_from = fate_presabs, values_from = count, values_fill = 0)

# Apply the function to each row (gene) in the table and perform a multiple test correction
p_values_fate_1 <- base::apply(BGC_fisher_fate_wide_1, 1, perform_fisher_test)

p_adjusted_BH_fate_1 <- p.adjust(p_values_fate_1, method = "BH")

# Combine gene names and p-values into a table
bgc_p_values_fate_1 <- data.frame(Contig = BGC_fisher_fate_wide_1$Contig, Adj_P_Value = p_adjusted_BH_fate_1, P_Value = p_values_fate_1) %>% 
  mutate(significant = ifelse(Adj_P_Value < 0.05, "significant", "not")) %>%
  mutate(sig_non_adj = ifelse(P_Value < 0.05, "significant", "not"))

# Print the table - see which genes are different between high and low resistant corals
print(bgc_p_values_fate_1)

# Add the p-values to the antimicrobials table
antimicrobials_with_pval_1 <- left_join(antimicrobials, bgc_p_values_fate_1, by = "Contig")

# How many of each type?
table(antimicrobials$Compound)

# Which compounds are most prevalent across all corals?
contig_prev_1 <- data.frame(Contig = rownames(antibac_presabs_1), 
                          N_coral_present = rowSums(antibac_presabs_1)) %>%
  left_join(antimicrobials, by = "Contig") %>%
  arrange(desc(N_coral_present))

# Look at the raw data for two of the significant taxa to see which fates it is most/least prevalent in
filter(BGC_fisher_fate_wide_1, Contig %in% c("k127_3355508", "k127_3314053"))
```
Cool...looks like with the stricter mean coverage cutoff of 1, now two of the bgcs were significantly differentially prevalent across disease fates: 
- k127_3355508 (amphotericin B) from Streptomyces nodosus, an antifungal antibiotic
- k127_3314053 (xenematide) from Xenorhabdus nematophila AN6/1, an antimicrobial compound
