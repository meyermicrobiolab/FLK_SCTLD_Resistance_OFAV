...
ggplot(bgc, aes(x = "", fill = Type_antismash)) +
  geom_bar(position = "fill") +
  scale_fill_manual(values = col)

ggplot(bgc, aes(x = On_contig_edge, fill = Type_antismash)) +
  geom_bar()+
  scale_fill_manual(values = col)
```
...
#ggsave("../figures/BGC_alluvial.pdf", width = 9, height = 12)
```
...
Are the compounds here more prevalent in the healthy compared to the diseased corals? Maybe just look for presence/absence rather than abundance since it is unclear what the actual activity would be. We are just looking for general abundance. 

I will use CoverM, which has a "contig" setting to investigate both the mean number of reads covering each position in the contig as well as the rpkm based abundances. From there, I will choose a threshold for presence/absence and calculate the prevalence of the BGC's in both the healthy as well as the diseased corals. I will look for which BGCs are more prevalent in either the healthy or diseased corals using a test I have done before that works with prevalence.  
...
annotation_col <- data.frame(
  Condition = metadata_bgc$Disease_on_colony_when_coring_binary)
rownames(annotation_col) <- colnames(antibac_presabs)

annotation_col$Condition <- factor(annotation_col$Condition, levels = c("Unaffected", "Diseased"))

# Custom colors for annotation
ann_colors <- list(
  Condition = c(Unaffected = "#0C7BDC", Diseased = "#FFC20A"),
  Activity_General = c(
    "antimicrobial" = "#990F26", 
    "antimicrobial, cytotoxic" = "#B33E52", 
    "antimicrobial precursor" = "#CC7A88",
    "antimicrobial defense" = "#E6B8BF", 
    "antibiotic resistance" = "#99600F",
    "cytotoxic" = "#B3823E", 
    "phytotoxic" = "#CCAA7A",
    "toxin" = "#E6D2B8",
    "virulence factor" = "#54990F"
  )
)
...
# Save heatmap to PDF
pdf("../figures/BGC_antimicrobial_heatmap.pdf", width = 13, height = 9)
pheatmap(antibac_presabs_forheatmap, 
        annotation_row = annotation_row, 
        annotation_colors = ann_colors,
        annotation_col = annotation_col, 
